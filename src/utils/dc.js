...
module.exports = {
  ...
  createButton: function (id, label, style, status) {
    const obj = new ButtonBuilder().setLabel(label).setStyle(style);
    if (style == ButtonStyle.Link) obj.setURL(id);
    else obj.setCustomId(id);
    if (status != undefined) obj.setDisabled(status);
    return obj;
  },
