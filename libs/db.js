 ...
 */
   ...
   */
    ...
    * @param {String} media Discord Attachment Link
    */
    insertYurestProfile: function (guild, user, charname, username, biography, prv, media) {
        const insert = db.prepare('INSERT INTO yurestprofile (guild, user, charname, username, private, bio, media) VALUES ( @guild, @user, @charname, @username, @biography, @prv, @media)');
        const insertMany = db.transaction((tags) => {
            for (const tag of tags)
                insert.run(tag);
        });
    ...
    /**
     * gibt Yurest Profil zurück
     * @param {String} username Profilename to search
     * @param {String} guild Guild Id
     * @return {Object} Profile data
     */
    getYurestProfile: function (guild, username) {
        const row = db.prepare(`SELECT * FROM yurestprofile WHERE username = ${username} AND guild = ${guild}`).get();
        return row;
    },
